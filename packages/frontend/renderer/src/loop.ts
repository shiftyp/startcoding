...
export const game = ({
  ...
  if (typeof MediaStreamTrackGenerator !== 'undefined') {
    const spriteVideo = renderElement = renderFrame.contentDocument!.createElement('video')
    ...
    const spriteCanvas = renderElement = renderFrame.contentDocument!.createElement('canvas')
  ...
  const gameLoop = () => {
    ...
    globals.minY = fromClientY(0)
    globals.maxY = fromClientY(height)

    const tick = { timing: { absTime: now, deltaTime }, globals: globals }
  ...
  setTimeout(gameLoop, 1000 / 60)
  renderFrame.contentWindow!.requestAnimationFrame(renderLoop)
  if (renderElement.tagName === 'VIDEO') (renderElement as HTMLVideoElement).play()
}
